...
public class DrinkWeb {
    ...
    @Operation(summary = "Fetches all Drinks", description = "When successful it fetches all drinks and returns a JSON-Code with status code 200.")
    ...
    @Operation(summary = "Updates the wished Drink", description = "When successful it updates the drink with the wished values and returns the JSON-Code of the updated drink with status code 200.")
    public ResponseEntity<Drinks> updateDrink(@Valid @PathVariable("reviewId")Integer id, @RequestBody Drinks drink) throws RestFoodException {
        return ResponseEntity.status(200).body(service.putADrink(drink, id));
    }

    @DeleteMapping(value = "/{drinkID}") //DELETE -- DELETE
    @PreAuthorize("hasAuthority('DELETE')")
    @Operation(summary = "Deletes the Review", description = "When successful it deletes the drink with status code 200.")
